...
async function generateMockTokens(
  deployer: Deployer,
  deployerAddress: string,
  aaveLandingPoolAddress: string
): Promise<Array<tokenData>> {
  const result: Array<tokenData> = [];
  for (let token of tokens) {
    ...
    await deployer.deploy(
      AToken,
      aaveLandingPoolAddress,
      DAIMockToken.address,
      18,
      `A${token.name}`,
      `a${token.symbol}`
    );
